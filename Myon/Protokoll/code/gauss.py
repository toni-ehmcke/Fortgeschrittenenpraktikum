...
def main():
    ...
    print "Mean Lifetime tau_ew without'+
	' combining channels = ",tau_ew, " us"
    L_min = np.min(L_eff)
    tau_sigma =  tau[np.abs(L_eff - L_min) < 1]
    dTau = np.max(np.abs(tau_ew - tau_sigma))
    print "With standarddeviation sigma_tau = ", dTau, " us"
    
    # combine neigboured channel-counts
    counts_gauss = np.zeros(len(counts) 
                            /2) 
    for i in range(len(counts_gauss)):
        counts_gauss[i] = counts[2*i] + counts[2*i+1]
    # channelwidth
    cWdth_gauss = 4./48        
    # measureable lifetimes                         
    t_gauss = cWdth_gauss * cNr[0:len(cNr)/2] - 1./48 
    # lower bond for channels  
    cMin_gauss = np.ceil(cMin / 2.) 
    # upper bond for channels                    
    cMax_gauss = np.ceil(cMax / 2.) 
    # total number of counted events                    
    N_gauss = np.sum(counts[cMin_gauss-1:              
                      cMax_gauss])
    # possible mean lifetimes in us
    tau_gauss = np.linspace(t_gauss[0],                 
                      t_gauss[cMax_gauss-1],1000) 
    # norming constant
    N0_gauss = getN0(tau_gauss, N_gauss, t_gauss, cWdth_gauss,
                     cMin_gauss, cMax_gauss)
    # effective likelihood-function
    L_eff_gauss = getLikelihood_gauss(tau_gauss,
		      t_gauss[cMin_gauss-1:cMax_gauss],
                      N0_gauss, cWdth_gauss, 
                      counts_gauss[cMin_gauss-1:cMax_gauss], N_gauss)  
   
    tau_ew_gauss = tau_gauss[np.argmin(L_eff_gauss)]
    print "Mean Lifetime tau_ew '+
	' with combined channels= ",tau_ew_gauss, " us"
    L_min_gauss = np.min(L_eff_gauss)
    tau_sigma_gauss =  tau_gauss[np.abs(L_eff_gauss 
				- L_min_gauss) < 1]
    dTau_gauss = np.max(np.abs(tau_ew_gauss - 
				tau_sigma_gauss))
    print "With standarddeviation sigma_tau = ", dTau_gauss, " us"

    
    
    fig = plt.figure("Mittlere Lebensdauer '+
			'von Myonen", figsize=(15,8))
    ...
    axHist.set_title(r'Mittlere Lebensdauer von'
			+' Myonen mit Gaussverteilung')
    axHist.plot(tau_gauss, L_eff_gauss)
    axHist.plot(tau, L_eff)
    axHist.text(3, 200000, r'Mittlere Lebensdauer mit'+
		'kombinierten Kanaelen $\tau_k = $'
		+ str(round(tau_ew_gauss,2)) 
		+ r'$\pm$'+ str(round(dTau_gauss,2))+ r' $\mu s$')
    axHist.text(3, 220000, r'Mittlere Lebensdauer ohne'+
		' kombinierte Kanaele $\tau = $' + str(round(tau_ew,2)) 
		+ r'$\pm$'+ str(round(dTau,2))	+ r' $\mu s$')
