...
def onMouseClick(event):
    """ Plot on each click one iteration of the newton method."""    
	...
	# plot the result of the iteration and substract the lifetime
	# of the first channel t[cMin-1]
        axHist.set_title(r'Mittlere Lebensdauer von Myonen mit 
			Exponentialfunktion $\tau =$' +
                        str(round(x_i * T - t[cMin-1],2)) + 
			r'$\pm$' + str(round(sigma_tau,2)) + 
			r' $\mu s$')
        plt.draw()
        x_i = x_ip1
...
print "T = ", T, " us"
sigma_tau = 1./N * np.sqrt(np.sum(counts[cMin-1:cMax] 
			* t[cMin-1:cMax]**2))
print sigma_tau
...
fig = plt.figure("Mittlere Lebensdauer von Myonen", 
		figsize=(15,8))
axHist = fig.add_subplot(111)
axHist.set_xlabel(r'$x = \frac{\tau}{T}$')
axHist.set_ylabel(r'$f(x)$')
axHist.set_xlim([0, 1])
axHist.set_title(r'Mittlere Lebensdauer von Myonen
